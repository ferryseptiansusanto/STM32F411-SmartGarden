...
void WaterQuality_ADC_Callback(ADC_HandleTypeDef *hadc) {
    // Pastikan interupsi berasal dari ADC yang benar
    if (hadc->Instance == sensor_hadc->Instance) {

        // Salin nilai DMA ke Buffer Aman secara instan (Double Buffering)
        // Karena ini berjalan di ISR, operasi ini tidak akan disela oleh RTOS Task.
        adc_safe_buffer[ADC_INDEX_TDS] = adc_dma_buffer[ADC_INDEX_TDS];
        adc_safe_buffer[ADC_INDEX_PH]  = adc_dma_buffer[ADC_INDEX_PH];

        // Beri tahu Task bahwa ada data baru yang siap diproses
        adc_data_ready = 1;
    }
}
...
void WaterQuality_ProcessAnalog(void) {
    // Jika tidak ada konversi data baru, jangan buang-buang siklus CPU
    if (!adc_data_ready) return;

    // Amankan pengambilan data dengan disable interrupt sesaat
    // Ini mencegah ISR menulis ulang adc_safe_buffer tepat saat kita sedang membacanya
    taskENTER_CRITICAL();
    uint16_t raw_TDS = adc_safe_buffer[ADC_INDEX_TDS];
    uint16_t raw_PH  = adc_safe_buffer[ADC_INDEX_PH];
    adc_data_ready = 0; // Reset flag
    taskEXIT_CRITICAL();

    // Pemrosesan Float (Berat untuk CPU, jadi AMAN dilakukan di luar ISR/Critical Section)
